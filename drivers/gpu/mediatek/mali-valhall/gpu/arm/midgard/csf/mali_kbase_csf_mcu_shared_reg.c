...
/* Trigger a fatal group error */
static void notify_group_csg_reg_map_error(struct kbase_queue_group *group)
{
...
static int userio_pages_replace_phys(struct kbase_device *kbdev, u64 vpfn, struct tagged_addr *phys)
{
	...
	if (unlikely(err) || !phys) {
		...
		update_mapping_with_dummy_pages(kbdev, vpfn, 2);
	}
	return err;
}
				   ...
				   struct kbase_csg_shared_region *csg_reg)
{
	...
	for (i = 0; i < nr_csis; i++) {
		...
		err = insert_dummy_pages(kbdev, vpfn, 2);
		if (unlikely(err))
...
fail_userio_pages_map_fail:
	while (i-- > 0) {
		vpfn = CSG_REG_USERIO_VPFN(reg, i, nr_susp_pages);
		kbase_mmu_teardown_pages(kbdev, &kbdev->csf.mcu_mmu, vpfn, 2, MCU_AS_NR);
	}
	vpfn = CSG_REG_PMOD_BUF_VPFN(reg, nr_susp_pages);
	kbase_mmu_teardown_pages(kbdev, &kbdev->csf.mcu_mmu, vpfn, nr_susp_pages, MCU_AS_NR);
fail_pmod_map_fail:
	vpfn = CSG_REG_SUSP_BUF_VPFN(reg, nr_susp_pages);
	kbase_mmu_teardown_pages(kbdev, &kbdev->csf.mcu_mmu, vpfn, nr_susp_pages, MCU_AS_NR);
fail_susp_map_fail:
...
int kbase_csf_mcu_shared_regs_data_init(struct kbase_device *kbdev)
{
	...
	const u32 nr_dummy_phys = MAX(nr_susp_pages, 2u);
	u32 i;
	int err;
